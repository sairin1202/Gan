...
def cal_gradient_penalty(dis,realData,fakeData):
    alpha=torch.rand(batchSize,1)
    alpha=alpha.expand(batchSize,realData.nelement()//batchSize).contiguous().view(batchSize,3,32,32)
    alpha=alpha.cuda()
    interpolates=alpha*realData+((1-alpha)*fakeData)
    interpolates=interpolates.cuda()
    interpolates=Variable(interpolates,requires_grad=True)
    disInterpolates=dis(interpolates)
    gradients=grad(outputs=disInterpolates,inputs=interpolates,grad_outputs=torch.ones(disInterpolates.size()).cuda(),create_graph=True,retain_graph=True,only_inputs=True)[0]
...
for epoch in range(trainEpoch):
    ...
    for i,data in enumerate(trainLoader):
        ...
        fakeLoss.backward(one)

        realImg=x
        disRealOutput=Discriminator(realImg)#real图片放入判别器
        realLoss=disRealOutput.mean()
        realLoss.backward(mone)#判别器Loss=D(真图片)-D(生成图片)

        gradient_penalty=cal_gradient_penalty(Discriminator,realImg.data,fakeImg.data)
        gradient_penalty.backward()
        disLoss=fakeLoss-realLoss+gradient_penalty#将两个loss相加
        ...
        if i%5==0:
            ...
            genLoss.backward(mone)#生成器Loss=-D(生成图片)
